...
import { Sliders, Calculator, ShieldCheck, HelpCircle, FileText, ArrowRight, TrendingDown } from 'lucide-react';
import { motion } from 'motion/react';

// Configuration profiles for calculation sectors
...
const MATERIAL_STANDARDS = [
  { id: 'standard', name: 'Standard Enterprise Certified', costFactor: 1.0, carbonFactor: 1.0, qualityRating: '99.92%' },
  { id: 'vanguard', name: 'Zero-Carbon E-Crete™ Eco-Vanguard', costFactor: 1.15, carbonFactor: 0.38, qualityRating: '99.98%' },
  { id: 'fortress', name: 'Seismic Base-Isolator Heavy Fortress', costFactor: 1.25, carbonFactor: 0.90, qualityRating: '99.99%' }
];
...
export default function FeasibilityCalculator() {
  ...
  const [totalArea, setTotalArea] = useState<number>(350000); // in sq ft
  const [storeys, setStoreys] = useState<number>(24);

  const activeSector = SECTORS[sector];
  const activeStandard = MATERIAL_STANDARDS.find(s => s.id === materialStandard) || MATERIAL_STANDARDS[0];

  // Perform Calculations
  const calculatedOutputs = useMemo(() => {
    const costFactor = activeStandard.costFactor;
    const carbonFactor = activeStandard.carbonFactor;

    // Total cost projection (Range +- 10%)
    const baseCost = totalArea * activeSector.baseCostPerSqFt;
    // Add height factors for tall properties
    const heightFactor = storeys > 40 ? 1.35 : storeys > 15 ? 1.15 : 1.0;
    const finalEstimatedBase = baseCost * costFactor * heightFactor;
    
    const costMinCr = Math.round((finalEstimatedBase * 0.95) / 10000000);
    const costMaxCr = Math.round((finalEstimatedBase * 1.05) / 10000000);

    // Concrete volume in cubic meters
    const totalConcrete = Math.round(totalArea * activeSector.baseConcreteMultiplier * (storeys > 15 ? 1.1 : 1.0));

    // Steel tendons requirements in tons
    const totalSteel = Math.round((totalArea / 10000) * activeSector.baseSteelMultiplier * (storeys > 30 ? 1.25 : 1.0));

    // CO2 Emissions reduced (measured in metric tons of carbon equivalent saved)
    const carbonDefaultCO2 = totalArea * 0.082; // average benchmark carbon output per sq ft
    const carbonSaved = Math.round(carbonDefaultCO2 * (1 - (0.72 * carbonFactor)));

    // Timeline calculation
    const baseTimeline = activeSector.baseTimelineMonths;
    const storeyAddition = Math.round(storeys * 0.25);
    const timeline = Math.round((baseTimeline + storeyAddition) * (materialStandard === 'fortress' ? 1.15 : 1.0));

    return {
      costRange: `₹${costMinCr} Cr - ₹${costMaxCr} Cr`,
      concreteVolume: `${totalConcrete.toLocaleString()} m³`,
      steelVolume: `${totalSteel.toLocaleString()} Metric Tons`,
      co2Saved: `${carbonSaved.toLocaleString()} Tons`,
      timelineMonths: `${timeline} Months`,
      reliabilityScore: activeStandard.qualityRating
    };
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="max-w-3xl mb-16">
          ...
          <h2 className="font-display text-3xl md:text-5xl font-bold tracking-tight text-white">
            Model Your Architectural Plan.
          </h2>
          <p className="mt-4 text-slate-400 text-base md:text-lg">
            Configure raw scope dimensions instantly. Our dynamic estimation equations parse core civil blueprints to draft real-world budgets, timing windows, and structural weights.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 bg-[#0D2248]/40 p-8 md:p-10 rounded-[20px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] border border-[rgba(255,107,44,0.3)] backdrop-blur-[10px] space-y-8">
                ...
                {Object.entries(SECTORS).map(([key, value]) => (
                  ...
                  >
                    <span className="block font-sans font-bold text-xs">
                      {value.name.split(' ').slice(0, 2).join(' ')}
                    </span>
                    <span className="block text-[10px] text-slate-500 font-mono mt-1 leading-normal truncate group-hover:text-slate-400">
                      {value.name}
                    </span>
                  </button>
                ))}
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-[10px] font-mono text-slate-500">
                <span>50K sq ft</span>
                <span>1M sq ft</span>
                <span>2.5M sq ft (Mega-scale)</span>
              </div>
            </div>

            {/* Step 3: Slider for Heights / Storeys */}
            <div className="space-y-3">
              <div className="flex justify-between items-center text-xs font-mono">
              ...
              <div className="flex justify-between text-[10px] font-mono text-slate-500">
                ...
                <span>Mid-Rise (40)</span>
                <span>Super-tall Skyscraper (100 storeys)</span>
              </div>
            </div>

            {/* Step 4: Quality & Materials formulation */}
            <div className="space-y-3">
              <label className="block text-xs font-mono font-bold uppercase tracking-wider text-slate-400">
                4. Select Material Spec & Quality Grade
              </label>
              <div className="space-y-3">
                  ...
                  >
                    <div>
                      ...
                      <span className="block text-[10px] font-mono text-slate-400 mt-0.5">
                        {std.id === 'vanguard' && 'Infused with Carbon-Negative E-Crete™ concrete technology'}
                        {std.id === 'fortress' && 'High structural resilience using custom pendulum core dampers'}
                        {std.id === 'standard' && 'Exceeds standard bureau building regulations under IS 456'}
                      </span>
                    </div>
          ...
          <div className="lg:col-span-5 bg-[#0D2248] text-white p-8 md:p-10 rounded-[20px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] border border-[rgba(255,107,44,0.3)] relative overflow-hidden flex flex-col justify-between self-stretch">
            {/* Gloss grid over overlay */}
            ...
            <div className="relative z-10">
              ...
              <div className="border-b border-white/10 pb-8 mb-8 space-y-1.5">
                ...
                <h4 className="font-display text-3xl md:text-4xl font-extrabold tracking-tight">
                  {calculatedOutputs.costRange}
                </h4>
                <p className="text-[10px] leading-relaxed text-slate-400">
                  Calculated based on local market factors, complex safety structures, structural damping and steel index.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-8">
                <div>
                  <span className="block font-mono text-[9px] text-slate-400 uppercase tracking-widest">Concrete Core</span>
                  <span className="block font-sans font-bold text-lg text-white mt-1">
                    {calculatedOutputs.concreteVolume}
                  </span>
                </div>
                <div>
                  <span className="block font-mono text-[9px] text-slate-400 uppercase tracking-widest">Structural Steel</span>
                  <span className="block font-sans font-bold text-lg text-white mt-1">
                    {calculatedOutputs.steelVolume}
                  </span>
                </div>
                <div>
                  <span className="block font-mono text-[9px] text-slate-400 uppercase tracking-widest">Duration</span>
                  <span className="block font-sans font-bold text-lg text-white mt-1">
                    {calculatedOutputs.timelineMonths}
                  </span>
                </div>
                <div>
            ...
            <div className="mt-12 pt-8 border-t border-white/5 space-y-4">
              ...
              <button
                onClick={() => {
                  const inquirySec = document.getElementById('inquiry-terminal');
                  if (inquirySec) {
