...
export default function Header() {
  ...
  return (
    <>
      <motion.header
        initial={{ backgroundColor: '#111844' }}
        animate={{ 
          backgroundColor: !isScrolled ? '#111844' : 'rgba(234, 224, 207, 0.85)',
          height: isScrolled ? '5rem' : '6rem'
        }}
        transition={{ duration: 0.3 }}
        className={`fixed top-0 left-0 right-0 z-50 flex items-center px-6 md:px-12 backdrop-blur-md border-b ${
          !isScrolled
            ? 'border-white/10 shadow-[0_8px_30px_rgba(17,24,68,0.15)]'
            : 'border-[#111844]/5 shadow-[0_4px_20px_rgba(17,24,68,0.02)]'
        }`}
      >
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center transition-all duration-300">
              ...
              return (
                <button
                  ...
                  className={`relative py-1 transition-all duration-300 cursor-pointer select-none rounded-none ${
                    isActive
                      ? (!isScrolled ? 'text-white' : 'text-[#4B5694]')
                      : (!isScrolled ? 'text-[#EAE0CF]/80 hover:text-white' : 'text-[#7288AE]/75 hover:text-[#4B5694]')
                  }`}
                >
                  {item.label}
                  {isActive && (
                    <motion.span 
                      ...
                      className={`absolute bottom-0 left-0 right-0 h-[2px] ${!isScrolled ? 'bg-white' : 'bg-[#4B5694]'}`}
          ...
          <div className="hidden lg:flex items-center">
            <button
              ...
              className={`px-8 py-3 text-sm font-sans font-bold uppercase tracking-widest transition-all duration-300 cursor-pointer rounded-none border ${
                !isScrolled 
                  ? 'border-[#EAE0CF]/30 text-white hover:bg-white hover:text-[#111844]' 
                  : 'border-[#111844] text-[#111844] hover:bg-[#4B5694] hover:border-[#4B5694] hover:text-white'
              }`}
            >
              Client Portal
            </button>
          ...
          <div className="flex items-center lg:hidden">
            <button
              ...
              className={`focus:outline-none transition-colors duration-300 ${!isScrolled ? 'text-white hover:text-[#EAE0CF]' : 'text-[#111844] hover:text-[#4B5694]'}`}
            >
          ...
          {mobileMenuOpen && (
            ...
            >
              ...
              <div className="pt-4 flex flex-col">
                <button
                  onClick={() => handleNavigate('/contact')}
                  className="w-full bg-[#111844] py-4 text-white font-sans font-bold text-sm uppercase tracking-widest rounded-none hover:bg-[#4B5694] transition-colors"
                >
                  Client Portal
                </button>
              </div>
